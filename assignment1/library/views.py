...
def login_page(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render(request,'library/login_page.html')
    else:
        return render(request, 'library/login_page.html')
...
def search_book(request):
    if request.method == 'POST':
        ...
        if Books.objects.filter(book_name=book_name).exists() :
           ...
           return render(request, 'library/Search_book.html', {'res':allbooks})
        else:
            messages.info(request, 'Book is Not Available in the library')
            logging.debug('Book is not in the list')
            return render(request, 'library/Homepage.html')
    else:
        return render(request, 'library/Search_book.html')
